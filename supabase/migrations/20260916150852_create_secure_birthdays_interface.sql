...
begin
  ...
  v_can_view_assigned_patients :=
    public.has_app_role('profissional') and v_professional_id is not null;

  if not (v_can_view_all_patients or v_can_view_assigned_patients) then
    raise exception 'Perfil sem autorização para consultar aniversariantes.' using errcode='42501';
  end if;
